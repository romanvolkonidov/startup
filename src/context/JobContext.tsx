...
export const JobProvider = ({ children }: { children: ReactNode }) => {
  ...
  const saveJob = async (jobId: string) => {
    ...
    try {
      ...
      // Optimistically update UI for this job only
      ...
      if (!res.success) {
        ...
        if (isCurrentlySaved) {
          ...
          if (jobToRestore) setSavedJobs(prev => [...prev, jobToRestore]);
        } else {
          setSavedJobs(prev => prev.filter(job => job.id !== jobId));
        }
        return res;
      }
      // Update savedBy for this job only
      setJobs(prev => prev.map(job =>
        job.id === jobId
          ? {
              ...job,
              savedBy: res.saved
                ? [...(job.savedBy || []).filter(id => id !== userId), userId]
